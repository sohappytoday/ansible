...
## 3. sudo / su / su -
부끄러운 얘기긴 하지만, 이 3가지의 차이점을 잘 몰랐다. 이번 기회를 통해 확실히 알게 되었다.  

발단은 특정 서버에서 `sudo -i`로 root 전환을 시도했는데, 해당 계정이 sudoers에 등록되어 있지 않아 계속 실패하는 상황이었다.  
그래서 `su -`를 써봤는데, 이번엔 비밀번호를 아무리 입력해도 `Authentication failure`가 떴다.  
알고 보니 `su -`는 **전환 대상 계정(root)의 비밀번호**를 요구하는데, 나는 계속 현재 내 계정 비밀번호를 치고 있었던 것이다.

이 경험으로 세 명령어의 차이를 확실히 정리하게 되었다.

`sudo -i` : 현재 계정에서 root 계정으로 로그인한 것처럼 실행, 내 계정 비밀번호를 넣어야 함 (현재 환경 유지)
`su` : root 계정으로 전환, root의 비밀번호를 넣어야 함 (현재 환경 유지)
`su -` : root 계정으로 완전히 전환, root의 비밀번호를 넣어야 함 (대상 계정 환경으로 초기화)
>물론 뒤에 다른 계정을 붙이면 다른 계정으로 접속 가능하나, 루트로 한정지어 설명하였다
...
### 4-3. 대용량 파일 전송 중 Control Plane OOM

위 4-2를 복구하던 중, CP의 이미지 tar(약 344MB)를 제어 노드로 가져오려고 Ansible `fetch` 모듈을 썼다.  
...
### 4-4. ubuntu 유저로 kubectl이 안 되던 문제
